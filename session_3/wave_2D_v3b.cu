...
__global__ void compute_V(DAT* Vx, DAT* Vy, DAT* P, const DAT dt, const DAT rho, const DAT dx, const DAT dy, const int nx, const int ny){
    for_ix for_iy
    if (iy<ny && ix>0 && ix<nx){ Velx(ix,iy) = Velx(ix,iy) - dt/dx/rho*(Pres(ix,iy)-Pres(ix-1,iy)); }
    if (iy>0 && iy<ny && ix<nx){ Vely(ix,iy) = Vely(ix,iy) - dt/dy/rho*(Pres(ix,iy)-Pres(ix,iy-1)); }
}
__global__ void compute_P(DAT* Vx, DAT* Vy, DAT* P, const DAT dt, const DAT k, const DAT dx, const DAT dy, const int nx, const int ny){
    for_ix for_iy
    if (iy<ny && ix<nx){ Pres(ix,iy) = Pres(ix,iy) - (dt*k/dx*(Velx(ix+1,iy)-Velx(ix,iy)) + dt*k/dy*(Vely(ix,iy+1)-Vely(ix,iy)) ); }
}
int main(){
    int i, it;
