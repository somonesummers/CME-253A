...
__global__ void compute_V(DAT* Vx, DAT* Vy, DAT* P, const DAT dt, const DAT rho, const DAT dx, const DAT dy, const int nx, const int ny){
    for_ix for_iy
    if (iy<ny && ix>0 && ix<nx){ Velx(ix,iy) = Velx(ix,iy) - dt*(Pres(ix,iy)-Pres(ix-1,iy))/dx/rho; }
    if (iy>0 && iy<ny && ix<nx){ Vely(ix,iy) = Vely(ix,iy) - dt*(Pres(ix,iy)-Pres(ix,iy-1))/dy/rho; }
}
__global__ void compute_P(DAT* Vx, DAT* Vy, DAT* P, const DAT dt, const DAT k, const DAT dx, const DAT dy, const int nx, const int ny){
    for_ix for_iy
    if (iy<ny && ix<nx){ Pres(ix,iy) = Pres(ix,iy) - dt*k*((Velx(ix+1,iy)-Velx(ix,iy))/dx + (Vely(ix,iy+1)-Vely(ix,iy))/dy ); }
}
int main(){
    int i, it;
    // Set up GPU
